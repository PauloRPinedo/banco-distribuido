...
OLD_EPOCH="$(curl -s "${OLD_URL}/admin/status" | "$PYTHON" -c 'import json,sys; print(json.load(sys.stdin)["epoch"])')"
...
for _ in $(seq 1 30); do
  ...
  if read -r NEW_ID NEW_URL < <(find_primary); then
    if [[ "$NEW_ID" != "$OLD_ID" ]]; then
      NEW_EPOCH="$(curl -s "${NEW_URL}/admin/status" | "$PYTHON" -c 'import json,sys; print(json.load(sys.stdin)["epoch"])')"
...
if [[ "$RESTART" -eq 1 ]]; then
  ...
  setsid "$PYTHON" -m bank.server --config "$CONFIG" --id "$OLD_ID" --data-dir "$DATA_DIR" \
    > "${DATA_DIR}/${OLD_ID}.out" 2>&1 < /dev/null &
